<div class="modal-dialog">
    <div class="modal-content">
        <div class="modal-header">
            <h5 class="modal-title" style="font-family: 'Cairo', sans-serif;">{{ __('supplier_trans.edit') }}</h5>
            ...
            <div class="modal-body">
                    ...
                    <div class="col-sm-12 col-md-6">
                        ...
                        <select wire:change="setCountryId($event.target.value)" id="country" class="custom-select">
                            ...
                            @foreach ($countries as $country)
                                <option value="{{ $country->id }}" @if ($country_id == $country->id) selected @endif>
                                    {{ $country->name }}</option>
                            @endforeach
                        </select>
                    </div>
                ...
                <div class="row pt-4">
                    <div class="col-sm-12 col-md-6">
                        ...
                        <select wire:change="setStateId($event.target.value)" id="state" class="custom-select">
                            <option value="0" selected disabled>{{ __('supplier_trans.state-select') }}
                            </option>
                            @foreach ($states as $state)
                                <option value="{{ $state->id }}" @if ($state_id == $state->id) selected @endif>
                                    {{ $state->name }}</option>
                            @endforeach
                        </select>
                    </div>
