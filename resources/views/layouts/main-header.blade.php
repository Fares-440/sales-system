            ...
            <ul class="nav navbar-nav ml-auto">
                ...
                <li class="nav-item dropdown ">
                    <a class="nav-link top-nav" data-toggle="dropdown" href="#" role="button"
                        ...
                        @if (auth()->user()->unreadNotifications->count())
                            {{-- <span class="badge badge-danger notification-status pulse">
                            </span> --}}
                    ...
                    <div class="dropdown-menu dropdown-menu-right dropdown-big dropdown-notifications">
                        <div class="dropdown-header notifications">
                            <a href="{{ route('posts.readAll') }}">
                                <strong>Notifications Read All</strong>
                            </a>
                            <span
                        ...
                        @foreach (auth()->user()->unreadNotifications as $notification)
                            <a href="{{ route('posts.show', $notification->data['post_id']) }}" class="dropdown-item">تم
                                اضافة تعليق جديد من قبل {{ $notification->data['createdByUser'] }} <small
                                    class="float-right text-muted time">{{ $notification->created_at->format('Y-m-d') }}</small>
                            </a>
                        @endforeach
                    </div>
                </li>
                ...
                <li class="nav-item dropdown mr-30">
                    ...
                    <div class="dropdown-menu dropdown-menu-right">
                        ...
                        <form method="POST" class="dropdown-item" action="{{ route('logout') }}">@csrf<i
                                class="text-danger ti-unlock"></i><input
                                style="background: none;border: none;cursor:pointer" type="submit" value="Logout">
                        </form>
                    </div>
                </li>
